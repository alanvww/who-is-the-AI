...
const ChatApp = () => {
  ...
  const ResponseCard = ({ resp, index, onVote, isSelected, isDisabled }: {
    resp: Response;
    index: number;
    onVote: () => void;
    isSelected: boolean;
    ...
    <div className="min-h-screen bg-gray-100 p-4 w-screen">
      <div className="max-w-4xl mx-auto">
        <Card className="w-full">
          <CardHeader>
            <CardTitle>
              <h1 className="text-4xl font-semibold text-center mb-4">Who is the AI?</h1>
            </CardTitle>
            <CardDescription>
              {gameState === 'joining' && "Join the game and vote for the reponse you think is generated by the AI player."}
                ...
                <div className="grid gap-4">
                  {responses.map((resp, index) => (
                    <ResponseCard
                      ...
                      resp={resp}
                      index={index}
                      onVote={() => handleVote(resp.playerId)}
                      isSelected={selectedResponse === resp.playerId}
